...
for i in range(len(c)):
    ...
    im = axs[j, i%2].imshow(imagen, extent=(x_min, x_max, y_min, y_max), 
                                           cmap='PuRd', origin='lower')
    cbar = fig.colorbar(im, ax=axs[j, i%2], shrink=0.7)
    cbar.set_label(f'Iteraciones antes de |z| > {R}')
    axs[j, i%2].set_title(f'{names[i]} \n c = {c[i].real}{c[i].imag:+}j', 
                          fontsize=13)
    axs[j, i%2].set_xlabel('Eje Real', fontsize=13)
    axs[j, i%2].set_ylabel('Eje Imaginario', fontsize=13)

plt.tight_layout()
plt.show()
fig.savefig('fig_Fractales_de_Julia_v1.png', dpi=300)
